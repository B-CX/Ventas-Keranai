...
export default async function AdminDashboard() {
  ...
  const ventasParaGrafico = await db.venta.findMany({
    take: 30,
  ...
  ventasParaGrafico.forEach(v => {
    const fechaStr = new Date(v.createdAt).toLocaleDateString('es-AR', { month: 'short', day: 'numeric' });
    ventasMap.set(fechaStr, (ventasMap.get(fechaStr) || 0) + v.total);
  });
  const ventasData = Array.from(ventasMap.entries()).map(([fecha, total]) => ({ fecha, total }));

  // 2. Productos más vendidos (Pie)
  const ventasPorProducto = await db.ventaItem.groupBy({
    by: ['varianteId'],
    _sum: { cantidad: true },
  ...
  // 3. Ingresos por vendedor (Bar)
  const ventasPorVendedor = await db.venta.groupBy({
    by: ['vendedorId'],
    _sum: { total: true },
  });
  const vendedoresIds = ventasPorVendedor.map(v => v.vendedorId);
  const vendedoresInfo = await db.user.findMany({ where: { id: { in: vendedoresIds } } });
  
  const vendedoresData = ventasPorVendedor.map(vv => {
    const v = vendedoresInfo.find(u => u.id === vv.vendedorId);
    return {
      name: v ? v.name : 'Desconocido',
      total: vv._sum.total || 0
    };
  });

  // 4. Funnel de ventas simulado
  const funnelData = [
    { name: 'Visitas (Estimado)', value: totalClientesCount === 0 ? 10 : totalClientesCount * 3 },
    { name: 'Clientes Registrados', value: totalClientesCount === 0 ? 5 : totalClientesCount },
    { name: 'Ventas Concretadas', value: totalVentasCount === 0 ? 2 : totalVentasCount },
  ];
