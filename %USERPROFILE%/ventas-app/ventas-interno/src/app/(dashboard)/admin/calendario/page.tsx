...
const COLORS = [
  { name: 'violeta', bg: 'bg-violet-600/20 border-violet-500/50 text-violet-300', dot: 'bg-violet-400' },
  { name: 'azul', bg: 'bg-blue-600/20 border-blue-500/50 text-blue-300', dot: 'bg-blue-400' },
  { name: 'verde', bg: 'bg-emerald-600/20 border-emerald-500/50 text-emerald-300', dot: 'bg-emerald-400' },
  { name: 'rojo', bg: 'bg-rose-600/20 border-rose-500/50 text-rose-300', dot: 'bg-rose-400' },
  { name: 'amarillo', bg: 'bg-amber-600/20 border-amber-500/50 text-amber-300', dot: 'bg-amber-400' },
  { name: 'gris', bg: 'bg-zinc-600/20 border-zinc-500/50 text-zinc-300', dot: 'bg-zinc-400' },
];
    ...
    <div className="min-h-screen p-6 text-white">
      ...
      <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold text-white">Calendario de Actividades</h1>
        ...
        <div className="flex items-center gap-2">
          <button
            onClick={handleToday}
            className="rounded-xl border border-white/10 bg-white/[0.04] px-4 py-2 text-sm font-medium hover:bg-white/[0.08] transition"
          >
            Hoy
          </button>
          <div className="flex items-center rounded-xl border border-white/10 bg-white/[0.04] p-0.5">
            ...
            <div className="relative flex items-center group cursor-pointer hover:bg-white/5 rounded-lg transition overflow-hidden">
              <input
                type="month"
              ...
              />
              <div className="flex items-center gap-2 px-3 py-1.5 text-sm font-semibold select-none">
      ...
      <div className="rounded-2xl border border-white/10 bg-white/[0.02] overflow-hidden shadow-2xl">
        ...
        <div className="grid grid-cols-7 border-b border-white/10 bg-white/[0.03]">
          {DAYS_OF_WEEK.map((d) => (
            <div key={d} className="py-2 text-center text-xs font-semibold uppercase tracking-wider text-zinc-400">
              {d}
            </div>
          ))}
        </div>
            ...
            {getGridDays().map((cell, idx) => {
              ...
              return (
                <div
                  ...
                  className={`min-h-[100px] border-r border-b border-white/5 p-2 transition hover:bg-white/[0.01] cursor-pointer flex flex-col gap-1 relative ${
                    cell.isCurrentMonth ? 'text-white' : 'text-zinc-600'
                  }`}
                >
                  {/* Número de día */}
